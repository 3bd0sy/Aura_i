...
# Optimized model training with reduced size
model = fasttext.train_supervised(
    input="train_data.txt",
    epoch=150,              # Reduced from 50 to 25
    dim=60,               # Reduced from 50 to 20
    lr=0.1,
    wordNgrams=6,
    bucket=300000,        # Reduced hash bucket size (default is 2M)
    loss='softmax',       # Using softmax instead of hierarchical softmax
    minn=2,              # Minimum length of char ngram
    maxn=6,              # Maximum length of char ngram
    minCount=2,          # Minimum number of word occurrences
    ws=5                 # Size of the context window
)

# Save the compressed model
model.save_model("intent_classifier_small.bin")
...
test_texts = [
    ...
    "how are you",
    "what is your name",
    "create image for a girl using anime style",
    " i want you to create image for me",
    "help me with generte image for syria",
    "what is the lastest news about syria",
...
print(
    f"\nModel size: {os.path.getsize('intent_classifier_small.bin') / (1024*1024):.2f} MB")
